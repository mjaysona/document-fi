...
export default function AddTransactionPage() {
  ...
  const queryParentTransactionId = String(
  ...
  const [returnedAllocatedFunds, setReturnedAllocatedFunds] = useState<TransactionListItem[]>([])
  ...
  >([])
  const [allocationParentData, setAllocationParentData] = useState<TransactionListItem[]>([])
  ...
  const form = useForm<TransactionFormValues>({
    initialValues: {
      ...
      isForAllocation: false,
      parentTransaction: null,
      allocatedFundType: 'completed',
    },
    validate: {
  ...
  const allocationParentSelectData = useMemo(() => {
    ...
    return allocationParentOptions
  }, [allocationParentOptions, form.values.parentTransaction, parentReferenceNumber])

  const isAllocationContext = Boolean(form.values.isAllocatedFund)
  ...
  const loadAllocatedFunds = async (parentId: string) => {
    ...
    const completedAllocatedTransactions = transactionsResult.success
      ? transactionsResult.data.filter(
          (tx) => tx.allocatedFundType === 'completed' && tx.transactionStatus === 'completed',
        )
      : []

    const returnedAllocatedTransactions = transactionsResult.success
      ? transactionsResult.data.filter((tx) => tx.allocatedFundType === 'returned')
      : []

    if (!transactionsResult.success) {
      setCompletedAllocatedFunds([])
      setReturnedAllocatedFunds([])
      setAllocatedFunds(0)
      return
    }
    ...
    setCompletedAllocatedFunds(completedAllocatedTransactions)
    setReturnedAllocatedFunds(returnedAllocatedTransactions)
  ...
  const hydrateFromTransaction = async () => {
    ...
    form.setValues({
      ...
      isForAllocation: tx.isForAllocation ?? false,
      parentTransaction: queryParentTransactionId || tx.parentTransaction || null,
      allocatedFundType: tx.allocatedFundType ?? undefined,
    })
    ...
    // If isAllocatedFund and allocatedFundType === 'returned', always use tx.sourceAccount
    if (tx.isAllocatedFund && tx.allocatedFundType === 'returned') {
      ...
      form.setFieldValue('sourceAccount', tx.sourceAccount ?? selectedAccount?.bankId ?? null)
    } else {
      form.setFieldValue('sourceAccount', tx.sourceAccount ?? null)
    }
    ...
    await loadAllocatedFunds(String(tx.id))

    const parentId = queryParentTransactionId || tx.parentTransaction || null
    if (parentId) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (queryParentTransactionId && !isEditMode) {
        const parentResult = await getTransactionById(queryParentTransactionId)
        if (!isMounted) return
        ...
        form.setFieldValue('parentTransaction', queryParentTransactionId)
        ...
        if (returnedFromTransactionId) {
          ...
          form.setFieldValue('to', prevAccount?.name ?? '')
        } else {
          form.setFieldValue('allocatedFundType', 'completed')
          form.setFieldValue('destinationAccount', null)
          form.setFieldValue('to', '')
          form.setFieldValue('sourceAccount', parentResult.data?.destinationAccount || null)
          form.setFieldValue('from', parentResult.data?.to || '')
        }
        setParentReferenceNumber(parentResult.data.referenceNumber ?? '')
        // Populate allocationParentOptions with the fetched parent so it displays in Select
        const parentOptionLabel = getAllocationParentOptionLabel(
          parentResult.data as TransactionListItem,
        )
        setAllocationParentOptions([
          {
            value: queryParentTransactionId,
            label: parentResult.data.referenceNumber?.trim() || 'NoRef',
            fullLabel: parentOptionLabel,
          },
        ])
        setAllocationParentData([parentResult.data as TransactionListItem])
      ...
      // Fix: If isAllocatedFund and allocatedFundType === 'returned', always use tx.sourceAccount
      ...
      const resolvedParentTransactionId = queryParentTransactionId || tx.parentTransaction || ''
      if (resolvedParentTransactionId) {
        const parentTxResult = await getTransactionById(resolvedParentTransactionId)
        if (!isMounted) return
        if (parentTxResult.success && parentTxResult.data?.referenceNumber) {
          ...
          // Populate allocationParentOptions with the fetched parent so it displays in Select
          const parentOptionLabel = getAllocationParentOptionLabel(
            parentTxResult.data as TransactionListItem,
          )
          setAllocationParentOptions([
            {
              value: resolvedParentTransactionId,
              label: parentTxResult.data.referenceNumber?.trim() || 'NoRef',
              fullLabel: parentOptionLabel,
            },
          ])
          setAllocationParentData([parentTxResult.data as TransactionListItem])

          // If queryParentTransactionId was passed, update parent-related fields based on that parent
          if (queryParentTransactionId && queryParentTransactionId !== tx.parentTransaction) {
            if (tx.allocatedFundType === 'returned') {
              const parentFinancialAccount = financialAccounts.find(
                (account) => account.id === parentTxResult.data?.financialAccount,
              )
              ...
              form.setFieldValue('sourceAccount', parentTxResult.data?.destinationAccount || null)
              form.setFieldValue('from', parentTxResult.data?.to || '')
              form.setFieldValue('destinationAccount', null)
              form.setFieldValue('to', '')
            }
          }
  ...
  }, [queryParentTransactionId, returnedFromTransactionId, isEditMode, transactionId])

  const navigateToChildTransaction = (childId: string) => {
    const parentId = String(transactionId || '').trim()
    const query = parentId ? `?allocateFrom=${encodeURIComponent(parentId)}` : ''
    router.push(`/app/records/transactions/${childId}/edit${query}`)
  }
  ...
  useEffect(() => {
    ...
    const debounceTimeout = window.setTimeout(() => {
      void (async () => {
        ...
        setAllocationParentData(result.data)
        setAllocationParentOptions(
  ...
  const createFormData = () => {
    ...
    const fallbackParentTransaction =
      form.values.isAllocatedFund && transactionId ? String(transactionId).trim() : ''
    ...
    if (form.values.allocatedFundType) {
      formData.append('allocatedFundType', form.values.allocatedFundType)
    }
    ...
    formData.append('isForAllocation', String(form.values.isForAllocation))
  ...
  const handleAllocatedFundChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isAllocated) {
      ...
      form.setFieldValue('transactionType', 'debit')
      form.setFieldValue('allocatedFundType', undefined)
      form.setFieldValue('destinationAccount', null)
      form.setFieldValue('to', '')
      ...
      form.setFieldValue('sourceAccount', parentFinancialAccount?.bankId || null)
      form.setFieldValue('from', parentFinancialAccount?.name || '')
    }
  }

  const handleParentTransactionChange = (selectedId: string | null) => {
    if (!selectedId) {
      form.setFieldValue('parentTransaction', queryParentTransactionId || null)
      form.setFieldValue('sourceAccount', null)
      form.setFieldValue('from', '')
    } else {
  ...
  return (
    <div className={classes.wrapper}>
      <div style={{ flex: 1, marginBottom: 16 }}>
                  ...
                  <Group>
                    <Group gap="xxs">
                      <Switch
                        ...
                        onChange={handleAllocatedFundChange}
                      />
                    ...
                    <Group gap="xxs">
                      <Switch
                        label="Is for allocation"
                        checked={form.values.isForAllocation}
                        onChange={(e) =>
                          form.setFieldValue('isForAllocation', e.currentTarget.checked)
                        }
                      />
                      <Tooltip label="Is for allocation - mark this as for allocation">
                        <span style={{ display: 'inline-flex', cursor: 'help' }}>
                          <CircleHelp size={14} />
                        </span>
                      </Tooltip>
        ...
        {(completedAllocatedFunds.length > 0 || returnedAllocatedFunds.length > 0) &&
          !isLoading &&
          (() => {
            ...
            return (
              <Stack mb="xl">
                                  ...
                                  <Table.Td>
                                    <span
                                      ...
                                      onClick={(event) => {
                                        ...
                                        navigateToChildTransaction(child.id)
                                      }}
                                    >
                ...
                {returnedAllocatedFunds.length > 0 &&
                  (() => {
                    return (
                      <Stack>
                        ...
                        <ScrollArea>
                          <Table withTableBorder withColumnBorders striped highlightOnHover>
                            ...
                            <Table.Tbody style={{ verticalAlign: 'top' }}>
                              {returnedAllocatedFunds.map((child) => (
                                <Table.Tr key={child.id}>
                                  <Table.Td>
                                    <span
                                      style={{
